...
\title{The Smithlab DNA Methylation Data Analysis Pipeline (DNMTools)}
...
dnmtools manuscript, listed first below. However, several improvements and
...
\item{\textbf{dnmtools:}} \bibentry{song2013reference}
...
DNMTools provides two methods for differential methylation (DM) analysis.
...
proportions for all target methylomes. DNMTools includes a program
...
lead to disease. Included in \prog{dnmtools} are three tools to
...
\begin{enumerate}
\item Find the script located in \texttt{DNMTOOLS_ROOT/src/utils} with the
name \texttt{bigWig\_to\_methcounts.py}. Here we use
\texttt{DNMTOOLS\_ROOT} to represent the path to the installation location
of \meth.
