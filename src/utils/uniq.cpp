...
std::default_random_engine e;           // NOLINT
std::uniform_int_distribution<int> di;  // NOLINT
...
struct uniq_summary {
  ...
  non_duplicate_fraction() const {
    return static_cast<double>(unique_reads - duplicate_reads) /
           static_cast<double>(std::max(1ul, total_reads));
  }
  ...
  duplication_rate() const {
    return static_cast<double>(reads_removed() + duplicate_reads) /
           static_cast<double>(std::max(1ul, duplicate_reads));
  }
...
process_inner_buffer(const bool add_dup_count,
  ...
  constexpr char du_tag[2] = {'D', 'U'};  // NOLINT(*-avoid-c-arrays)
  const std::size_t n_reads = std::distance(it, jt);
  const std::size_t selected = uniq_random::rand() % n_reads;

  // NOLINTBEGIN(*-narrowing-conversions,*-array-to-pointer-decay)
  if (add_dup_count) {
  ...
  // NOLINTEND(*-narrowing-conversions,*-array-to-pointer-decay)
}
...
process_buffer(const bool add_dup_count, rd_stats &rs_out,
               ...
               std::vector<bamxx::bam_rec> &buffer,
               const bamxx::bam_header &hdr, bamxx::bam_out &out) {
...
uniq(const bool add_dup_count, const std::uint32_t max_buffer_size,
     const std::int32_t n_threads, const std::string &cmd,
...
int
main_uniq(int argc, char *argv[]) {  // NOLINT(*-avoid-c-arrays)
  try {
    ...
    std::size_t the_seed = 408;  // NOLINT
    std::string outfile;
    std::string statfile;
    std::string histfile;
