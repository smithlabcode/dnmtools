/*
 *    Copyright (C) 2009-2022 University of Southern California and
 *                       Andrew D. Smith
 *
...
is_cpg(const std::string &s, size_t i) {
  return (i < (s.length() - 1)) && is_cytosine(s[i]) && is_guanine(s[i + 1]);
}
...
separate_regions(const std::vector<T> &big_regions,
                 const std::vector<U> &regions, const std::vector<V> &seqs,
