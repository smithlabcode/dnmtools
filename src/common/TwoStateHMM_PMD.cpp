...
// NOLINTBEGIN(*-avoid-magic-numbers,*-owning-memory,*-narrowing-conversions)
...
TwoStateHMM::TransitionPosteriors_rep(
  ...
  const vector<size_t> &reset_points, const vector<double> &start_trans,
  const vector<vector<double>> &trans, const vector<double> &end_trans,
  const vector<double> &fg_alpha, const vector<double> &fg_beta,
  const vector<double> &bg_alpha, const vector<double> &bg_beta,
  const vector<bool> &array_status, const size_t transition,
  ...
  return TransitionPosteriors_rep(
    values, reset_points, start_trans[0], start_trans[1], trans[0][0],
    trans[0][1], end_trans[0], trans[1][0], trans[1][1], end_trans[1],
...
TwoStateHMM::forward_algorithm_rep(
  const vector<vector<pair<double, double>>> &vals, const size_t start,
  const size_t end, const double lp_sf, const double lp_sb, const double lp_ff,
  const double lp_fb, const double lp_ft, const double lp_bf,
...
TwoStateHMM::estimate_transitions_rep(
  const vector<vector<pair<double, double>>> &vals, const size_t start,
  const size_t end, const vector<pair<double, double>> &f,
  const vector<pair<double, double>> &b, const double total,
  const vector<unique_ptr<EmissionDistribution>> &fg_distro,
  const vector<unique_ptr<EmissionDistribution>> &bg_distro, const double lp_ff,
  const double lp_fb, const double lp_bf, const double lp_bb,
  vector<double> &ff_vals, vector<double> &fb_vals, vector<double> &bf_vals,
...
double
TwoStateHMM::single_iteration_rep(
  const vector<vector<pair<double, double>>> &values,
  const vector<vector<double>> &vals_a_reps,
  const vector<vector<double>> &vals_b_reps, const vector<size_t> &reset_points,
  vector<pair<double, double>> &forward, vector<pair<double, double>> &backward,
  double &p_sf, double &p_sb, double &p_ff, double &p_fb, double &p_ft,
  ...
  assert(isfinite(lp_sf) && isfinite(lp_sb) && isfinite(lp_ff) &&
         isfinite(lp_fb) && isfinite(lp_ft) && isfinite(lp_bf) &&
         isfinite(lp_bb) && isfinite(lp_bt));
  ...
  for (size_t i = 0; i < reset_points.size() - 1; ++i) {
    const double score = forward_algorithm_rep(
      values, reset_points[i], reset_points[i + 1], lp_sf, lp_sb, lp_ff, lp_fb,
      lp_ft, lp_bf, lp_bb, lp_bt, fg_distro, bg_distro, forward);

    const double backward_score = backward_algorithm_rep(
      values, reset_points[i], reset_points[i + 1], lp_sf, lp_sb, lp_ff, lp_fb,
      lp_ft, lp_bf, lp_bb, lp_bt, fg_distro, bg_distro, backward);

    if (DEBUG &&
        (fabs(score - backward_score) / max(score, backward_score)) > 1e-10) {
    ...
    estimate_transitions_rep(values, reset_points[i], reset_points[i + 1],
                             ...
                             lp_ff, lp_fb, lp_bf, lp_bb, ff_vals, fb_vals,
                             bf_vals, bb_vals);
...
TwoStateHMM::BaumWelchTraining_rep(
  ...
  const double score = BaumWelchTraining_rep(
    values, reset_points, start_trans[0], start_trans[1], trans[0][0],
    trans[0][1], end_trans[0], trans[1][0], trans[1][1], end_trans[1],
...
TwoStateHMM::BaumWelchTraining_rep(
  const vector<vector<pair<double, double>>> &values,
  const vector<size_t> &reset_points, double &p_sf, double &p_sb, double &p_ff,
  double &p_fb, double &p_ft, double &p_bf, double &p_bb, double &p_bt,
  vector<unique_ptr<EmissionDistribution>> &fg_distro,
  vector<unique_ptr<EmissionDistribution>> &bg_distro,
  ...
  if (VERBOSE) {
    cerr << "MAX_ITER=" << max_iterations << "\tTOLERANCE=" << tolerance
         << '\n';
    cerr << setw(5) << "ITR" << setw(10) << "F size" << setw(10) << "B size"
         << setw(14) << "DELTA\n";
  }
    ...
    if (array_status[r]) {
      for (size_t i = 0; i < values[0].size(); ++i) {
        if (values[r][i].second > 0) {
          vals_a_reps[r][i] = log(min(
            max(values[r][i].first / values[r][i].second, 1e-2), 1.0 - 1e-2));
          vals_b_reps[r][i] =
            log(1.0 - min(max(values[r][i].first / values[r][i].second, 1e-2),
                          1.0 - 1e-2));
        }
      }
    }
      ...
      for (size_t i = 0; i < values[0].size(); ++i) {
        if (values[r][i].first + values[r][i].second >= 1) {
          vals_a_reps[r][i] = log(min(
            max(values[r][i].first / (values[r][i].first + values[r][i].second),
                1e-2),
            1.0 - 1e-2));
          vals_b_reps[r][i] =
            log(1 - min(max(values[r][i].first /
                              (values[r][i].first + values[r][i].second),
                            1e-2),
                        1.0 - 1e-2));
        }
      }
  ...
  for (size_t i = 0; i < max_iterations; ++i) {
    ...
    double total = single_iteration_rep(
      values, vals_a_reps, vals_b_reps, reset_points, forward, backward,
      p_sf_est, p_sb_est, p_ff_est, p_fb_est, p_ft_est, p_bf_est, p_bb_est,
      p_bt_est, fg_distro, bg_distro);
    ...
    if (DEBUG) {
      cerr << "S_F\t" << p_sf_est << '\n'
           ...
           << '\n';
      for (size_t r = 0; r < NREP; ++r)
        cerr << "Emission parameters for Rep" << r + 1 << setw(14)
             << fg_distro[r]->getalpha() << setw(14) << fg_distro[r]->getbeta()
             << setw(14) << bg_distro[r]->getalpha() << setw(14)
             << bg_distro[r]->getbeta() << '\n';
    }

    if (VERBOSE) {
      cerr << setw(5) << i + 1 << setw(10) << 1 / p_fb_est << setw(10)
           << 1 / p_bf_est << setw(14) << total << setw(14) << prev_total
           << setw(14) << total - prev_total << setw(14)
...
TwoStateHMM::PosteriorScores_rep(
  ...
  const vector<double> &fg_alpha, const vector<double> &fg_beta,
  const vector<double> &bg_alpha, const vector<double> &bg_beta,
  const vector<bool> &classes, vector<double> &llr_scores,
  const vector<bool> &array_status) const {
  vector<unique_ptr<EmissionDistribution>> fg_distro;
  vector<unique_ptr<EmissionDistribution>> bg_distro;
...
TwoStateHMM::PosteriorDecoding_rep(
  ...
  const vector<double> &fg_alpha, const vector<double> &fg_beta,
  const vector<double> &bg_alpha, const vector<double> &bg_beta,
  vector<bool> &classes, vector<double> &llr_scores,
  const vector<bool> &array_status) const {
  vector<unique_ptr<EmissionDistribution>> fg_distro;
  vector<unique_ptr<EmissionDistribution>> bg_distro;
...
TwoStateHMM::TransitionPosteriors_rep(
  ...
  const vector<unique_ptr<EmissionDistribution>> &fg_distro,
  const vector<unique_ptr<EmissionDistribution>> &bg_distro,
  const size_t transition, vector<double> &scores) const {
  size_t NREP = values.size();
  const double lp_sf = log(p_sf);
  const double lp_sb = log(p_sb);
  const double lp_ff = log(p_ff);
  ...
  for (size_t i = 0; i < values[0].size(); ++i) {
    if (i == reset_points[j]) {
      ...
      const double fg_to_fg_state =
        forward[i - 1].first + lp_ff + fg_vals + backward[i].first;
      const double fg_to_bg_state =
        forward[i - 1].first + lp_fb + bg_vals + backward[i].second;
      const double bg_to_fg_state =
        forward[i - 1].second + lp_bf + fg_vals + backward[i].first;
      const double bg_to_bg_state =
        forward[i - 1].second + lp_bb + bg_vals + backward[i].second;
      const double denom =
...
TwoStateHMM::PosteriorDecoding_rep(
  ...
  const vector<unique_ptr<EmissionDistribution>> &fg_distro,
  const vector<unique_ptr<EmissionDistribution>> &bg_distro,
  vector<bool> &classes, vector<double> &llr_scores) const {
  double total_score = 0;
...
// NOLINTEND(*-avoid-magic-numbers,*-owning-memory,*-narrowing-conversions)
