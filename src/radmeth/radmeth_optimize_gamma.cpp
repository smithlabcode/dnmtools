...
template <typename T>
static void
gradient(const gsl_vector *params, Regression<T> &reg, gsl_vector *output) {
  ...
  const auto n_samples = reg.n_samples();
  for (auto i = 0u; i < n_samples; ++i) {
    const double y = mc[i].n_meth;  // ADS: n_meth maybe int or float
    const auto n = mc[i].n_reads;
...
neg_loglik(const gsl_vector *params, void *object) {
  auto reg = static_cast<Regression<T> *>(object);
...
neg_loglik_and_grad(const gsl_vector *params, void *object, double *loglik_val,
  ...
  *loglik_val = neg_loglik<T>(params, object);
  neg_gradient<T>(params, object, d_loglik_val);
}
...
fit_regression_model(Regression<T> &r, std::vector<double> &p_estimates,
  ...
  auto loglik_bundle = gsl_multimin_function_fdf{
    &neg_loglik<T>,              // objective function
    &neg_gradient<T>,            // gradient
    &neg_loglik_and_grad<T>,     // combined obj and grad
...
fit_regression_model_gamma(Regression<std::uint32_t> &r,
                           ...
                           double &dispersion_estimate) {
  fit_regression_model<std::uint32_t>(r, p_estimates, dispersion_estimate);
}
