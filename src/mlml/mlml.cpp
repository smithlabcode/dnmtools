...
static int
binom_null(const double alpha, const std::size_t n, const double p_hat,
           const double p) {
  // ADS: function tests if final argument p is outside the (1-alpha)
  // CI for p_hat given counts n.
  double lower{};
  double upper{};
  wilson_ci_for_binomial(alpha, static_cast<double>(n), p_hat, lower, upper);
  if (p < upper && p > lower)
    return 0;
  else
    return 1;
...
log_L(const size_t h, const size_t g, const size_t m, const size_t l,
  ...
  double log_lkhd =
    lnchoose(h + g, h) + lnchoose(m + l, m) + lnchoose(u + t, u);

  if (p_h > 0)
    log_lkhd += h * log(p_h);
  if (p_h < 1)
    log_lkhd += g * log(1 - p_h);

  if (p_m > 0)
    log_lkhd += m * log(p_m);
  if (p_m < 1)
    log_lkhd += l * log(1 - p_m);

  if (p_h + p_m < 1)
    log_lkhd += u * log(1 - p_h - p_m);
  if (p_h + p_m > 0)
    log_lkhd += t * log(p_h + p_m);
...
static void
get_start_point(const size_t t, const size_t u, const size_t m, const size_t l,
                const size_t h, const size_t g, const double tolerance,
...
static void
expectation(const size_t a, const size_t x, const double p, const double q,
...
static double
maximization(const size_t x, const size_t y, const size_t a, const size_t b,
...
static double
update_p_m(const size_t x, const size_t y, const size_t z, const size_t w,
...
expectation_maximization(const bool DEBUG, const size_t x, const size_t y,
                         const size_t z, const size_t w, const size_t a,
                         const size_t b, const double tolerance, double &p,
                         double &q) {
  constexpr auto max_iterations = 500;

  if (DEBUG) {
    cerr << "t:" << a << ", u:" << b << ", m:" << z << ", l:" << w
         << ", h:" << y << ", g:" << x << '\n'
         << "p:" << p << ", q:" << q << '\n';
  }
  ...
  if (DEBUG) {
    cerr << iter << '\t' << "p_m=" << p << '\t' << "p_h=" << q << '\t'
         << "log-likelihood=" << log_L(y, x, z, w, b, a, q, p) << '\n';
  }
}
...
expectation_maximization(const bool DEBUG, const size_t x, const size_t y,
  ...
  if (DEBUG) {
    cerr << iter << '\t' << "p=" << p << '\t' << "q=" << q << '\t'
         << "log-likelihood=" << log_L(x, y, z, w, p, q) << '\n';
  }
}
...
static void
process_three_types(const double alpha, const double tolerance, const bool FLAG,
                    const string &hydroxy_file, const string &bs_seq_file,
                    const string &oxbs_file, const string &outfile,
                    const string &out_methcount_pseudo_h,
                    const string &out_methcount_pseudo_m, size_t &total_sites,
                    size_t &overshoot_sites, size_t &conflict_sites) {
  constexpr auto max_read_count = 500;

  if (!check_file_non_empty(hydroxy_file))
    throw std::runtime_error("input file empty: " + hydroxy_file);

  ifstream h_in(hydroxy_file);
  if (!h_in)
    throw std::runtime_error("failed to open input file: " + hydroxy_file);

  if (!check_file_non_empty(bs_seq_file))
    throw std::runtime_error("input file empty: " + bs_seq_file);

  ifstream b_in(bs_seq_file);
  if (!b_in)
    throw std::runtime_error("failed to open input file: " + bs_seq_file);

  if (!check_file_non_empty(oxbs_file))
    throw std::runtime_error("input file empty: " + oxbs_file);

  ifstream o_in(oxbs_file);
  if (!o_in)
    throw std::runtime_error("failed to open input file: " + oxbs_file);
  ...
  if (!out_methcount_pseudo_m.empty()) {
    ...
    if (!out_m)
      throw std::runtime_error("failed to open output file: " +
                               out_methcount_pseudo_m);
  }
  if (!out_methcount_pseudo_h.empty()) {
    out_h.open(out_methcount_pseudo_h);
    if (!out_h)
      throw std::runtime_error("failed to open output file: " +
                               out_methcount_pseudo_h);
  }

  MSite r, s, o;
  while (h_in >> r && b_in >> s && o_in >> o) {
    if (r.n_reads > max_read_count)
      r.n_reads = max_read_count;
    const size_t h = r.n_meth();
    const size_t g = r.n_unmeth();

    if (s.n_reads > max_read_count)
      s.n_reads = max_read_count;
    const size_t t = s.n_meth();
    const size_t u = s.n_unmeth();

    if (o.n_reads > max_read_count)
      o.n_reads = max_read_count;
    const size_t m = o.n_meth();
    const size_t l = o.n_unmeth();

    assert(r.chrom == s.chrom && r.chrom == o.chrom && r.pos == o.pos &&
           r.pos == s.pos);

    total_sites++;

    if ((h + g > 0 && u + t > 0) || (h + g > 0 && m + l > 0) ||
        (m + l > 0 && u + t > 0)) {
      auto x = g;
      auto y = h;
      auto z = m;
      ...
      auto p_h_hat = static_cast<double>(y) / (x + y);
      auto p_m_hat = static_cast<double>(z) / (z + w);
      auto p_u_hat = static_cast<double>(b) / (a + b);

      // use frequent method result if no overshoot
      if (p_h_hat + p_m_hat + p_u_hat == 1.0) {
        out << r.chrom << '\t' << r.pos << '\t' << r.strand << '\t' << r.context
            << '\t' << p_m_hat << '\t' << p_h_hat << '\t' << p_u_hat << '\t'
            << 0 << '\n';

        // write out pseudo methcount files for mC and hmC
        if (!out_methcount_pseudo_m.empty())
          out_m << r.chrom << '\t' << r.pos << '\t' << r.strand << '\t'
                << r.context << '\t' << p_m_hat << '\t'
                << (a + b + x + y + z + w) << '\n';
        if (!out_methcount_pseudo_h.empty())
          out_h << r.chrom << '\t' << r.pos << '\t' << r.strand << '\t'
                << r.context << '\t' << p_h_hat << '\t'
                << (a + b + x + y + z + w) << '\n';
      }
      else {
        ...
        p_u = 1 - p_m - p_h;
        if (p_h <= 2.0 * tolerance)
          p_h = 0.0;
        if (p_m <= 2.0 * tolerance)
          p_m = 0.0;
        if (p_u <= 2.0 * tolerance)
          p_u = 0.0;
        if (p_m >= 1.0 - 2.0 * tolerance)
          p_m = 1.0;
        if (p_h >= 1.0 - 2.0 * tolerance)
          p_h = 1.0;
        if (p_u >= 1.0 - 2.0 * tolerance)
          p_u = 1.0;

        int CONFLICT{};
        if (p_h_hat + p_m_hat + p_u_hat != 1 && FLAG) {
          auto cflt_h = binom_null(alpha, x + y, p_h_hat, p_h);
          auto cflt_m = binom_null(alpha, z + w, p_m_hat, p_m);
          auto cflt_u = binom_null(alpha, a + b, p_u_hat, p_u);
          CONFLICT = cflt_m + cflt_u + cflt_h;
        }

        out << r.chrom << '\t' << r.pos << '\t' << r.strand << '\t' << r.context
            << '\t' << p_m << '\t' << p_h << '\t' << p_u << '\t' << CONFLICT
            << '\n';
        ...
        if (!out_methcount_pseudo_m.empty())
          out_m << r.chrom << '\t' << r.pos << '\t' << r.strand << '\t'
                << r.context << '\t' << p_m << '\t' << (a + b + x + y + z + w)
                << '\n';
        if (!out_methcount_pseudo_h.empty())
          out_h << r.chrom << '\t' << r.pos << '\t' << r.strand << '\t'
                << r.context << '\t' << p_h << '\t' << (a + b + x + y + z + w)
                << '\n';
      }
    }
      ...
      if (!out_methcount_pseudo_m.empty()) {
        int coverage = m + l;
        const double level =
          coverage > 0 ? static_cast<double>(m) / coverage : 0.0;
        out_m << r.chrom << '\t' << r.pos << '\t' << r.strand << '\t'
              << r.context << '\t' << level << "\t" << coverage << '\n';
      }
      if (!out_methcount_pseudo_h.empty()) {
        const int coverage = h + g;
        const double level =
          coverage > 0 ? static_cast<double>(h) / coverage : 0.0;
        out_h << r.chrom << '\t' << r.pos << '\t' << r.strand << '\t'
              << r.context << '\t' << level << '\t' << coverage << '\n';
      }
    }
...
static void
process_two_types(const double alpha, const double tolerance, const bool FLAG,
                  const string &hydroxy_file, const string &bs_seq_file,
                  const string &oxbs_file, const string &outfile,
                  const string &out_methcount_pseudo_h,
                  const string &out_methcount_pseudo_m, size_t &total_sites,
  ...
  if (oxbs_file.empty()) {
    ...
    if (!f_in)
      throw std::runtime_error("failed to open file: " + hydroxy_file);
    s_in.open(bs_seq_file);
    if (!s_in)
      throw std::runtime_error("failed to open file: " + bs_seq_file);
  }
    ...
    s_in.open(oxbs_file);
    if (!s_in)
      throw std::runtime_error("failed to open file: " + oxbs_file);
  }
  else {
    f_in.open(oxbs_file);
    if (!f_in)
  ...
  while (f_in >> f && s_in >> s) {
    if (f.chrom != s.chrom || f.pos != s.pos)
      throw std::runtime_error("error: sites not synchronized between files");

    if (f.n_reads > max_read_count)
      f.n_reads = max_read_count;
    size_t x = f.n_meth();
    size_t y = f.n_unmeth();
    if (f_rev)
      std::swap(x, y);
    ...
    if (x + y > 0 && z + w > 0) {
      ...
      if (x / static_cast<double>(x + y) + z / static_cast<double>(z + w) <=
          1.0) {
        ...
        if (p >= 1.0 - 2.0 * tolerance)
          p = 1.0;
        if (q >= 1.0 - 2.0 * tolerance)
          q = 1.0;
        if (r >= 1.0 - 2.0 * tolerance)
          r = 1.0;

        if (FLAG) {
          const double p_hat1 = static_cast<double>(x) / (x + y);
          auto cflt1 = binom_null(alpha, x + y, p_hat1, p);
          const double p_hat2 = static_cast<double>(z) / (z + w);
          auto cflt2 = binom_null(alpha, z + w, p_hat2, q);
          CONFLICT = cflt1 + cflt2;
        }
      }

      out << f.chrom << '\t' << f.pos << '\t' << f.strand << '\t' << f.context
          << '\t';
      if (oxbs_file.empty())
        out << r << '\t' << p << '\t' << q << '\t';
      else if (hydroxy_file.empty())
      ...
      if (!out_methcount_pseudo_h.empty()) {
        ...
        out_h << s.chrom << '\t' << s.pos << '\t' << s.strand << '\t'
              << s.context << '\t' << level << '\t' << coverage << '\n';
      }
...
int
main_mlml(int argc, char *argv[]) {  // NOLINT(*-avoid-c-arrays)
  try {
    bool VERBOSE = false;
    bool FLAG = true;
    string oxbs_file;
    ...
    opt_parse.add_opt("output", 'o', "output file (default: stdout)", false,
                      outfile);
    opt_parse.add_opt("bsseq", 'u', "input BS-seq methcounts file", false,
                      bs_seq_file);
    opt_parse.add_opt("tabseq", 'h', "input TAB-seq methcounts file", false,
                      hydroxy_file);
    opt_parse.add_opt("oxbsseq", 'm', "input oxBS-seq methcounts file", false,
                      oxbs_file);
    opt_parse.add_opt("tolerance", 't', "EM convergence threshold", false,
                      tolerance);
    opt_parse.add_opt("alpha", 'a',
                      "significance level of binomial test for each site",
                      false, alpha);
    opt_parse.add_opt("outh", 'H',
                      "hmC pseudo methcount output file (default: null)", false,
                      out_methcount_pseudo_h);
    opt_parse.add_opt("outm", 'M',
                      "mC pseudo methcount output file (default: null)", false,
                      out_methcount_pseudo_m);
    ...
    if (!hydroxy_file.empty() && !bs_seq_file.empty() && !oxbs_file.empty())
      process_three_types(alpha, tolerance, FLAG, hydroxy_file, bs_seq_file,
                          oxbs_file, outfile, out_methcount_pseudo_h,
                          out_methcount_pseudo_m, total_sites, overshoot_sites,
                          conflict_sites);
    else
      process_two_types(alpha, tolerance, FLAG, hydroxy_file, bs_seq_file,
                        oxbs_file, outfile, out_methcount_pseudo_h,
                        out_methcount_pseudo_m, total_sites, overshoot_sites,
                        conflict_sites);

    if (VERBOSE)
      cerr << "total sites: " << total_sites << '\n'
           << "sites with overshoot: " << overshoot_sites << " ("
           << 1.0 * overshoot_sites / total_sites * 100 << "%)\n"
           << "sites conflicting to at least two input: " << conflict_sites
           << " (" << 1.0 * conflict_sites / total_sites * 100 << "%)\n";
  }
