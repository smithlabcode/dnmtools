...
struct Triplet {
  double hypo, HYPER, HYPO;
};

class ThreeStateHMM {
public:
  ThreeStateHMM(std::vector<std::pair<double, double>> &obs,
                const std::vector<size_t> &res, const double tol,
                const size_t max_itr, const bool v);

  void
  set_parameters(const betabin &hypo_em, const betabin &HYPER_em,
                 const betabin &HYPO_em,
                 const std::vector<std::vector<double>> &tr);

  void
  get_parameters(betabin &hypo_em, betabin &HYPER_em, betabin &HYPO_em,
  ...
  std::vector<double> hypo_log_likelihood, HYPER_log_likelihood,
    HYPO_log_likelihood;
  ...
  std::vector<double> hypo_hypo, hypo_HYPER, HYPER_hypo, HYPER_HYPER,
    HYPER_HYPO, HYPO_HYPER, HYPO_HYPO;
